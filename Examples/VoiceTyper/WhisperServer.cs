...
namespace VoiceTyper
{
	...
	sealed class WhisperServer : IDisposable
	{
		...
		{
			var file = new ByteArrayContent( WavBytes( mono16k, 16000 ) );
			file.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue( "audio/wav" );
			return PostInference( file, "audio.wav", langCode );
		}
		...
		{
			using var fs = File.OpenRead( path ); // streamed, so multi-GB inputs don't get buffered in RAM
			var file = new StreamContent( fs );
			file.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue( "application/octet-stream" );
			return PostInference( file, Path.GetFileName( path ), langCode );
		}

		string PostInference( HttpContent file, string fileName, string langCode )
		{
			Process? p; int pt;
			...
			using HttpResponseMessage resp = http.PostAsync( $"http://127.0.0.1:{pt}/inference", form ).GetAwaiter().GetResult();
